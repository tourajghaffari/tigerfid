...
//---------- testing LSI
ErrorStatus setDateTimeTest()
{
    ErrorStatus status = SUCCESS;
    
    uint8_t rtcDate[4] = {0x0b,   // 11
                          0x18,   // 24
                          0x06,   // 06
                          0x05};  // friday
    
    uint8_t rtcTime[3] = {0x00,   // seconds
                          0x05,   // mins
                          0x016}; // hour        
    /***    
    RTC_DateStructInit(date);
    date->RTC_WeekDay = rtcDate[3];  //((uint8_t)0x05);
    date->RTC_Month = rtcDate[2];    //((uint8_t)0x06);
    date->RTC_Date = rtcDate[1];     //24;
    date->RTC_Year = rtcDate[0];     //11;

    RTC_TimeStructInit(time);
    time->RTC_Hours   = rtcTime[2];  //0x16;
    time->RTC_Minutes = rtcTime[1];  //0x05;
    time->RTC_Seconds = rtcTime[0];  //0x00;
    ***/
...
ErrorStatus readTime()
{
    ErrorStatus status = SUCCESS;
       
    // read Date, time from EEPROM
    status = readDateEE(&date);
    if (status == ERROR)
        return status;
