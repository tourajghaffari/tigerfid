...
{
    uint8_t  batt = (Battery_GetVoltage() >> 1) & 0xFE;

    if (Battery_HasLowLevel() == TRUE)
    {
        lowVoltBatteryDetected = TRUE;
        updateBatteryVoltage(BATTERY_VOLT_LOW | batt);
    ...
    else
    {
        ...
        updateBatteryVoltage(BATTERY_VOLT_NORMAL | batt);
    }
}
...
void rfCommandIRQHandler()
{
    ...
    if (commandStatus == COMMAND_STATUS_RECEIVED)
    {
        ...
        updateInterruptStatus((Band_GetState() == OPEN) ? IS_TAMPER_BAND_OPENED : IS_TAMPER_BAND_CLOSED, FALSE);
        commandHandler();
    }
    else
    {
...
ErrorCode commandHandler()
{
    ...
    else if (command == READ_SERIAL_NUMBER)
    {
        if (rtcConfigured == TRUE) {
            status = ERROR_RTC_RUNNING;
        } else {
            status = SerialNumber_Store();
        }
        setCommandStatus(status, FALSE);
        return (status);
    }
    else if (command == READ_RF_UID)
    {
        if (rtcConfigured == TRUE) {
            status = ERROR_RTC_RUNNING;
        } else {
    ...
    updateInterruptStatus((Band_GetState() == OPEN) ? IS_TAMPER_BAND_OPENED : IS_TAMPER_BAND_CLOSED, updateHistory);
...
ErrorCode commandStartTimers()
{
    ...
    else if ((interruptStatus & IS_TAMPER_BAND_CLOSED) == IS_TAMPER_BAND_CLOSED)
    {
        status = configureRTC();
        if (status == ERROR_NONE)
        {
...
ErrorCode commandReadBatteryVoltage()
{
    ...
    batt   = (Battery_GetVoltage() >> 1) & 0xFE;
    batt  |= (lowVoltBatteryDetected == TRUE) ? BATTERY_VOLT_LOW : BATTERY_VOLT_NORMAL;
    status =  updateBatteryVoltage(batt);
...
{
    const  uint8_t    commandStatus = COMMAND_STATUS_UNKNOWN;
    const  uint8_t    empId[2]      = { 0x00, 0x00 };
    const  uint8_t    command       = NOOP;
    const  uint8_t    tagVersion    = TAG_VERSION;
           uint8_t    batt;
           ErrorCode  status;
    ...
    status = updateInterruptStatus((Band_GetState() == OPEN) ? IS_TAMPER_BAND_OPENED : IS_TAMPER_BAND_CLOSED, FALSE);
    if (status != ERROR_NONE)
        return status;

    lowVoltBatteryDetected = Battery_HasLowLevel();
