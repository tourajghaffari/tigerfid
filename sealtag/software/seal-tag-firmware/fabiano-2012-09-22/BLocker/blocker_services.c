...
typedef  struct  serial_number {
    unsigned char  Hdr[2];
    unsigned char  SerialID[4];
} SERIAL_NUMBER;

__eeprom __no_init const SERIAL_NUMBER eSerial;
...
ErrorCode commandHandler()
{
    ...
    else if ((command >= READ_SERIAL_0) &&
             (command <= READ_SERIAL_7))
    {
        if ((eSerial.Hdr[0] == 'A') &&
            (eSerial.Hdr[1] == 'W')) {
            uint8_t ix     = command - READ_SERIAL_0;
            uint8_t nibble = (1 - (ix & 1)) * 4;

            ix >>= 1;

            status = (ErrorCode)(0x40 | ((eSerial.SerialID[ix] >> nibble) & 0xF));
        } else if ((eSerial.Hdr[0]      == 0) &&
                   (eSerial.Hdr[1]      == 0) &&
                   (eSerial.SerialID[0] == 0) &&
                   (eSerial.SerialID[1] == 0) &&
                   (eSerial.SerialID[2] == 0) &&
                   (eSerial.SerialID[3] == 0)) {
            status = (ErrorCode)0x50;   // blank serial number
        } else {
            status = (ErrorCode)0x51;   // invalid serial number
        }
...
ErrorCode commandReadBatteryVoltage()
{
    ...
    status = updateBatteryVoltage((lowVoltBatteryDetected == TRUE) ? BATTERY_VOLT_LOW : BATTERY_VOLT_NORMAL);
